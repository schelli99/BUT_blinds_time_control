...
class BlindsCover(CoverEntity, RestoreEntity):
    ...
    async def sun_state_changed_event(self, event):
        """Adapter for async_track_state_change_event (new HA API)."""
        entity_id = event.data.get("entity_id")
        old_state = event.data.get("old_state")
        new_state = event.data.get("new_state")
        await self.sun_state_changed(entity_id, old_state, new_state)
    ...
    async def async_added_to_hass(self):
        # Listen to all state changes (used for up/down switch feedback and night lights)
        self._unsub_state_changed = self.hass.bus.async_listen(
            "state_changed", self._handle_state_changed
        )

        # Periodic add-ons timer (1 minute)
        self._unsub_add_ons = async_track_time_interval(
            self.hass, self.add_ons, timedelta(minutes=1)
        )

        # Track next dawn/dusk sensors (new HA API)
        self._unsub_sun_dawn = async_track_state_change_event(
            self.hass, ["sensor.sun_next_dawn"], self.sun_state_changed_event
        )
        self._unsub_sun_dusk = async_track_state_change_event(
            self.hass, ["sensor.sun_next_dusk"], self.sun_state_changed_event
        )
    ...
    async def async_will_remove_from_hass(self):
        for unsub in (
            getattr(self, "_unsub_state_changed", None),
            getattr(self, "_unsub_add_ons", None),
            getattr(self, "_unsub_sun_dawn", None),
            getattr(self, "_unsub_sun_dusk", None),
            getattr(self, "_unsubscribe_auto_updater", None),
        ):
            if unsub:
                unsub()
